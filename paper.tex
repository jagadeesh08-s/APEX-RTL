...
\begin{abstract}
Writing efficient Register-Transfer Level (RTL) code is crucial in modern VLSI design, as early-stage code quality heavily influences the final Power, Performance, and Area (PPA) of the integrated circuit. Traditional electronic design automation (EDA) flows only reveal design bottlenecks after logic synthesis and physical design runs, which can take hours or days for complex circuits. This paper introduces APEX-RTL, a lightweight machine learning-based assistant that evaluates RTL Verilog code quality before logic synthesis and automatically generates optimized code refactorings. APEX-RTL parses Verilog code, extracts a 21-dimensional structural feature vector (incorporating a Place-and-Route routing congestion proxy), and employs trained Ridge, Random Forest, Gradient Boosting, XGBoost, and LightGBM models to estimate PPA. The framework is trained on dual technology node datasets (NanGate 45nm bulk planar and ASAP7 7nm FinFET) consisting of 1,000 synthesized designs each. Additionally, the framework computes a Design Quality Score (DQS) driven by feature attributions and generates optimized Verilog code using automated rewrite templates. Our experiments demonstrate that our models achieve high prediction accuracy with $R^2$ scores exceeding 0.992 (Area), 0.987 (Power), and 0.983 (Delay) under NanGate 45nm, and 0.996 (Area), 0.989 (Power), and 0.990 (Delay) under ASAP7 7nm. Evaluation across a 5-design benchmark suite shows that APEX-RTL successfully refactors logic structures, delivering a 7.9\% timing delay reduction on arithmetic datapath critical paths.
\end{abstract}

\begin{IEEEkeywords}
RTL Code Quality, VLSI Design, PPA Prediction, Explainable AI, Automated Code Refactoring, Place-and-Route Congestion, FinFET, Machine Learning.
\end{IEEEkeywords}
...
However, major research gaps remain:
\begin{enumerate}
...
To validate APEX-RTL's ability to drive code optimization, we analyzed the auto-refactoring flow across a 5-design benchmark suite (Table \ref{tab:comparison}).
...
The refactoring engine successfully flattened nested conditionals (raising the Priority Encoder DQS from 85.0 to 100.0) and inserted pipeline register stages in arithmetic structures (halving the multiplier combinational logic delay from 6.85ns to 3.42ns). In all cases, routing congestion factors were minimized.
